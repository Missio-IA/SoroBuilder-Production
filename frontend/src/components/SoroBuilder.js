...
function SoroBuilder() {
  ...
  const handleExit = () => {
    const confirmExit = window.confirm(
      'Are you sure you want to exit? 1 credit has been deducted for accessing Sorobuilder.'
    );
    if (confirmExit) {
      navigate('/generate');
    }
  ...
  useEffect(() => {
    ...
    // Initialize Pusher
    const pusherInstance = new Pusher('e695aba01bee94c8cfcc', {
      cluster: 'eu'
    });
  ...
  const handleRestartSession = () => {
    ...
    const newPusherInstance = new Pusher('e695aba01bee94c8cfcc', {
      cluster: 'eu'
    });
      ...
      <header className="flex items-center justify-between p-3 bg-black bg-opacity-30 backdrop-blur-lg">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="bg-gray-800 border-gray-700">
            ...
            <DropdownMenuItem 
              onClick={handleExit} 
              className="text-red-500 hover:bg-red-700 cursor-pointer"
            >
              Exit
            </DropdownMenuItem>
          </DropdownMenuContent>
